...
**항목 본문의 블록 라벨은 반드시 `#### 라벨` h4 헤딩으로 쓸 것.** 뷰어(index.html)는 h4 헤딩으로 블록(어떤 연구 / 초록 / 주요 결과 / 적용 알고리즘 / 국내 vs 해외 / 출처 등)을 인식한다. `**볼드 라벨**`이나 `라벨: 내용` 평문 방식으로 쓰면 앱에서 본문이 접힌 채 헤드라인만 보이는 문제가 생긴다 (2026-08-14 8개 회차 일괄 수정의 원인). 예:

```markdown
### paper: 헤드라인 (저널 YYYY-MM)
