...
Read `C:\Users\USER\Documents\GitHub\study\EDITORIAL-PRINCIPLES.md`. Sections 4 (categories), 5 (length per category), and 6 (markdown structure) are canonical — follow them exactly. If the file disagrees with this prompt, follow the file (it may have been updated).

Required sections in order:
1. `# YYYY-MM-DD (Day N)` h1 with date and Day N
2. `> <one-line title>` blockquote (e.g., "본인 학습용 — Day 1: 신장 · 내분비/당뇨 · 심혈관")
3. `## SPECIALTY_<KEY>` for each specialty in the day's group (3 sections on Day 1/2, 4 on Day 3). Each MUST contain **exactly 7 items in this order**:
   - `### paper: <headline 1>` — top-tier journal paper #1
   - `### paper: <headline 2>` — top-tier journal paper #2 (different topic — e.g., positive trial #1 vs negative/safety trial #2, or core-specialty #1 vs adjacent-field #2). If genuinely no second paper this cycle, write `### paper: (이번 회차 신규 사항 없음)` and omit body — but try to find one.
...
4. `## CASE` (optional, ~once per week — typically Day 1) — single hypothetical clinical scenario, `### <case headline>` followed by 1~2 paragraphs of natural prose (병력·검사·감별진단·의사결정 woven into prose, no bold markers required). Mention which SPECIALTY item it ties to. Must end with the disclaimer: "(가상 시나리오, 실제 환자 정보 아님)".
5. `## SELF_CHECK` — 2 or 3 questions in `### Q1.` / `### Q2.` form. Body is multiple-choice options on separate lines (e.g., `a) ...`, `b) ...`, `c) ...`), then `#### 답·해설` block with the correct option in **bold** (`**b)**`) and a 1~2 sentence rationale.
...
- Day 1/2 (3 specialties × 7) = **21 items**
- Day 3 (4 specialties × 7) = **28 items**
...
Construct the full markdown following the format spec above. Length target: total file ~6,000~9,000 characters of body content. Per item 200~400 chars on expanded body.

Avoid duplication: a topic that was covered within the past 30 days should only re-appear if there is a genuine new development (additional reporting, policy reaction, price/dose update, regulatory action). Plain re-statement of yesterday's news is not allowed. Check by reading recent files in `briefings/` matching the past 30 days.
